// Auto-generated from PBS: 2026-07-31 11:30:38
...
const pokemonData = [
  ...
  },
  {
    ...
    name: "Lomprigus",
    types: ["Ghost", "Fire"],
    category: "Haunted Lamp Pokémon",
    description: "Lomprigus traps wandering spirits inside its lamp-like body. Its flames grow brighter whenever it hears frightened voices nearby.",
    isRegionalForm: false
  },
  {
    dex: "064",
...
const regionalFormData = [
  {
    ...
    id: "CHINGLING,1",
    imageId: "chingling-1",
    name: "Chingling - Inflamous",
    types: ["Normal"],
    category: "Clock Bell Pokémon",
    description: "This regional form resembles a ticking clock. Its chimes follow a precise rhythm that never falls out of sync.",
    isRegionalForm: true
  },
  {
    dex: "FORM",
  ...
  },
  {
    ...
    id: "VICTINI,1",
    imageId: "victini-1",
    name: "Victini - Inflamous Corrupted",
    types: ["Lunar", "Electric"],
    category: "Category not yet assigned",
    description: "A Victini corrupted by Team Midnight's experiments. Its once pure victory energy has become unstable Dark and Electric power that overwhelms everything it touches.",
    isRegionalForm: true
  },
  {
    dex: "FORM",
  ...
  },
  {
    ...
    name: "Pueppey - Mexian",
    types: ["Water"],
    category: "Corrupted Pup Pokémon",
    description: "In the Mexian variant, Pueppey’s body absorbed toxic waste from polluted waters, mutating its elemental balance into unstable water and poison energy.",
    isRegionalForm: true
  },
  {
    dex: "FORM",
    ...
    name: "Pueppey - Zonean",
    types: ["Grass"],
    category: "Zone Bloom Pup Pokémon",
    description: "A Zonean adaptation of Pueppey that evolved in dense flora regions. It channels unpredictable growth surges in battle.",
    isRegionalForm: true
  },
  {
    dex: "FORM",
  ...
  },
  {
    ...
    id: "BLITZLE,1",
    imageId: "blitzle-1",
    name: "Blitzle - Inflamous",
    types: ["Ground"],
    category: "Desert Foal Pokémon",
    description: "Adapted to arid regions, its hooves harden to traverse dunes. Static electricity has faded, replaced by control over shifting sands.",
    isRegionalForm: true
  },
  {
    dex: "FORM",
...
function imageName(mon) {
  return mon.imageId || mon.id.toLowerCase();
}
  ...
  card.innerHTML = `
    <div class="dex-card-header">
      <span class="dex-number">
        ${mon.isRegionalForm ? "Regional Form" : `#${mon.dex}`}
      </span>
      <h3>${mon.name}</h3>
    </div>
    ...
    <p class="dex-description">${mon.description}</p>
...
function renderCards(grid, list, emptyMessage) {
  if (!grid) {
    return;
  }
  ...
  if (list.length === 0) {
    grid.innerHTML = `<p class="no-results">${emptyMessage}</p>`;
    return;
  }
...
function ensureRegionalFormsSection() {
  let section = document.getElementById("regionalFormsSection");

  if (section) {
    return section.querySelector("#regionalFormsGrid");
  }

  const mainGrid = document.getElementById("pokedexGrid");
  ...
  section.id = "regionalFormsSection";

  section.innerHTML = `
    <h2>Inflamous Regional Forms</h2>
    <p>Custom regional forms detected from pokemon_forms.txt.</p>
    <div id="regionalFormsGrid" class="dex-grid"></div>
  `;

  const mainSection = mainGrid.closest("section");

  if (mainSection && mainSection.parentNode) {
    mainSection.parentNode.insertBefore(section, mainSection.nextSibling);
  }
...
function matchesSearch(mon, query) {
  return [
    ...
    mon.category,
    mon.description,
    mon.isRegionalForm ? "regional form" : "fakemon"
  ]
    .join(" ")
    .toLowerCase()
    .includes(query);
...
function renderAll(query = "") {
  const mainGrid = document.getElementById("pokedexGrid");
  const regionalGrid = ensureRegionalFormsSection();

  const filteredMain = pokemonData.filter(mon => matchesSearch(mon, query));
  const filteredRegional = regionalFormData.filter(mon => matchesSearch(mon, query));

  renderCards(mainGrid, filteredMain, "No Pok\u00E9mon matched your search.");
  renderCards(regionalGrid, filteredRegional, "No regional forms matched your search.");
}
...
function setupSearch() {
  ...
  searchBox.addEventListener("input", () => {
    renderAll(searchBox.value.trim().toLowerCase());
  });
}
